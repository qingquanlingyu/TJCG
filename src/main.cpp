...
int main()
{
    ...
    glfwSetInputMode(window, GLFW_CURSOR, GLFW_CURSOR_DISABLED);//???毓??? 

    glEnable(GL_DEPTH_TEST);//???????炔???
    glDepthFunc(GL_LESS);
    ...
    if (SphereVAO == 0)
    {
        ...
        for (unsigned int y = 0; y <= Y_SEGMENTS; ++y)
        {
            for (unsigned int x = 0; x <= X_SEGMENTS; ++x)
            {
                ...
                positions.push_back(glm::vec3(xPos, yPos, -zPos));//??两????????-z??????????效??????
                normals.push_back(glm::vec3(xPos, yPos, -zPos));//??两????????-z??????????效??????
            }
        }
        ...
        glEnableVertexAttribArray(0);
        glVertexAttribPointer(1, 3, GL_FLOAT, GL_FALSE, 6 * sizeof(float), (void*)(3 * sizeof(float)));//????量
        glEnableVertexAttribArray(1);
    }
    glBindVertexArray(SphereVAO);
