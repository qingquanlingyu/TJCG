...
int main()
{
    ...
    // --------------------
    GLFWwindow* window = glfwCreateWindow(SCR_WIDTH, SCR_HEIGHT, "Anakin", NULL, NULL);
    if (window == NULL)
    {
    ...
    // -------------------------
    Shader shaderGeometryPass("../res/vs/ssao_geometry.vs", "../res/fs/ssao_geometry.fs");
    Shader shaderLightingPass("../res/vs/ssao.vs", "../res/fs/ssao_lighting.fs");
    Shader shaderSSAO("../res/vs/ssao.vs", "../res/fs/ssao.fs");
    Shader shaderSSAOBlur("../res/vs/ssao.vs", "../res/fs/ssao_blur.fs");
    Shader shaderSimpleDepth("../res/vs/3.1.3.shadow_mapping_depth.vs", "../res/fs/3.1.3.shadow_mapping_depth.fs");
    Shader shaderLightBox("../res/vs/8.1.deferred_light_box.vs", "../res/fs/8.1.deferred_light_box.fs");
    ...
    // load textures
    // -------------
    unsigned int woodTexture = loadTexture("../res/textures/wood2.jpg");

    // load models
    // -----------
    // Model backpack("../xres/objects/backpack/backpack.obj");
    // Model ourModel("../res/model/untitled.obj");
    Model ourModel("../res/objects/airplane/scene.gltf");
    ...
    auto m_ssao = new SSAO(SCR_WIDTH, SCR_HEIGHT, GBUFFER_TEXTURE_NUM);
    auto m_depthMapFBO = new DepthMapFBO();
    ...
    shaderGeometryPass.setInt("texture_specular1", 0);
    ...
    while (!glfwWindowShouldClose(window))
    {
        ...
        shaderSSAO.use();
        m_ssao->ActivateSSAOTexture(shaderSSAO);
        ...
        glClear(GL_COLOR_BUFFER_BIT);
        shaderSSAOBlur.use();
        m_ssao->ActivateSSAOBlurTexture(shaderSSAOBlur);
        renderQuad();
        glBindFramebuffer(GL_FRAMEBUFFER, 0);


        // 5. lighting pass: traditional deferred Blinn-Phong lighting with added screen-space ambient occlusion
        // -----------------------------------------------------------------------------------------------------
        glClear(GL_COLOR_BUFFER_BIT | GL_DEPTH_BUFFER_BIT);
        shaderLightingPass.use();
        ...
        glBindTexture(GL_TEXTURE_2D, m_depthMapFBO->getDepthMap());
        renderQuad();
        ...
        // 6. render lights on top of scene
        // --------------------------------
        shaderLightBox.use();
